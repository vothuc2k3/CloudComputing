...
if (isset($_POST['txtName'])) :
    ...
    if (isset($_POST['btnDel'])) :
        $cid = $_POST['txtID'];
        $sqlInsert = "DELETE FROM `002_category` WHERE cat_id =?";
        $stmt = $db_link->prepare($sqlInsert);
        $execute = $stmt->execute(array("$cid"));
        if ($execute) {
    ...
    <form id="form1" name="form1" method="post" action="" class="form-horizontal" role="form">
        ...
        <div class="form-group pb-3">
            <div class="col-sm-offset-2 col-sm-10">
                <input type="submit" class="btn btn-primary" name="<?php echo (isset($_GET["cat_id"])) ? "btnEdit" : "btnDel"; ?>" id="btnAction" value='<?php echo (isset($_GET["cat_id"])) ? "Edit" : "Delete"; ?>' />
